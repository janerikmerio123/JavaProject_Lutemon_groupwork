...
public class HomeFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        RadioGroup homeGroup = view1.findViewById(R.id.CBhome);
        ArrayList<Lutemon> lutemons = storage.getLutemons();

        List<CheckBox> cb = new ArrayList<CheckBox>();

        for (Lutemon lut : lutemons) {
            if (lut.getPlaceOfLutemon() == 0) {
                checkBox = new CheckBox(this.getContext());
                checkBox.setText(lut.getName());
                checkBox.setId(i);
                homeGroup.addView(checkBox);
                i++;
                cb.add(checkBox);
            }
        }

        Button button = (Button) view1.findViewById(R.id.moveChosen2);
        button.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                //place your action here
                RadioGroup radioGroupHome = view1.findViewById(R.id.radioGroupHome);
